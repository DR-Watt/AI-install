...
infra_require() {
  ...
  local done_key="MOD_${mod_num}_DONE"
  local done_val
...
dialog_warn() {
  ...
  log "WARN" "$1"
  if [ "$GUI_BACKEND" = "yad" ]; then
    yad --title="$title" --text="$msg" --image=dialog-warning \
      --button="OK:0" \
      --width=780 --center --on-top 2>/dev/null
  else
    whiptail --title "$title" --msgbox "$msg" "$h" "$WT_W"
...
# STEP_INTERACTIVE=false esetén automatikusan igen-t ad.
ask_proceed() {
  local prompt="${1:-Folytatjuk?}"

  # Automatikus mód (pl. CI futtatáshoz)
