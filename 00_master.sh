#!/bin/bash
# =============================================================================
# 00_master.sh — Vibe Coding Workspace Installer v6.5.1
#
# Változtatások v6.5.1 (2026-04-12 terminál hiba fix):
#
#   BUG FIX: line 118: 01a: value too great for base
#     Tünet: sudo bash 00_master.sh → bash aritmetikai hiba az induláskor
#     Gyökérok: Az üdvözlő dialog welcome loop (line ~119) hivatkozik az
#       ${INFRA_REBOOT[$_id]:-} tömbre, de a 'declare -A INFRA_REBOOT=(…)'
#       deklaráció JÓVAL KÉSŐBB volt a masterben (checklist szekciónál).
#       A deklaráció nélkül bash NUMERIKUS indexű tömbnek tekinti az INFRA_REBOOT-ot.
#       Amikor $_id="01a", bash $(( 01a )) kiértékelést kísérel meg:
#         → "01a" octal szám lenne (kezdő 0 miatt)
#         → de 'a' nem oktális szám (max 7) → "value too great for base"
#     Fix: declare -A INFRA_REBOOT és declare -A INFRA_DEP
#       ÁTHELYEZVE a welcome loop ELÉ (source "$REGISTRY" utáni közvetlen helyre).
#       Korábban: checklist szekciónál volt → TÚL KÉSŐ
#       Most: source "$REGISTRY" után azonnal → welcome loop ELŐTT
#
...
# =============================================================================
# INFRA META-ADATOK — [v6.5.1 FIX: DEKLARÁCIÓ HELYE]
# =============================================================================
# FONTOS: Ezeknek a deklarációknak a welcome loop ÉS a checklist ELŐTT kell
# lenniük. Ha UTÁNA lennének, bash numerikus tömbnek tekintené INFRA_REBOOT-ot
# és INFRA_DEP-et, és az "01a" kulcs aritmetikai kiértékelést okozna:
#   bash: $(( 01a )) → "01a: value too great for base (error token is "01a")"
#
# Gyökér: bash octal szabály — 0-val kezdődő szám oktális (01a nem valid octal)
# INFRA_REBOOT: reboot szükséges ezek után (label jelzés, nem hardcode döntés)
...
# MEGJEGYZÉS: INFRA_REBOOT és INFRA_DEP deklarációk FENTEBB vannak (v6.5.1 fix)
...
for _id in "${INFRA_IDS[@]}"; do
  ...
  # INFRA_REBOOT itt már deklarált asszociatív tömb → string index: nincs aritmetikai hiba
...
# MEGJEGYZÉS: INFRA_REBOOT és INFRA_DEP már fentebb deklarálva (v6.5.1 fix)
